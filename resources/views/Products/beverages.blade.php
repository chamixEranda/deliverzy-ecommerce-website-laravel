@extends('layouts.app')

@section('title', 'Categories-Beverages - ')
    
@section('content')
    <section style="padding-top: 80px">
        <div class="container">
            <h2 class="text">Beverages</h2>
        </div>
    
        <div class="modal fade" id="qtyModal" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true">
            <div class="modal-dialog">
                <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title deleteTitle" id="staticBackdropLabel">Delete Product</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
                    <form  method="POST" id="QuantityForm" class="mt-3"> 
                        @csrf
                        @method('delete')
                        <div class="modal-footer">
                        <button type="submit" class="btn btn btn-dark  btn-md rounded-0 ps-4 pe-4 adminOptionBtn">YES</button>
                        </div>
                    </form>
                </div>
                </div>
            </div>
        </div>
     
        
    
         <!-- card -->
            <div class="container-fluid mb-5 " style="padding-left: 80px;" data-aos="fade-down">
            <div class="row" id="ProCard" style="cursor: pointer;">
                  @foreach ($product as $oneproduct)
                    <div class="col-md-2 mt-4 ms-4 ">             
                        <div class="card index-item pt-2"> 
                        @auth
                            @if (in_array($oneproduct->id,Auth::user()->wishlists->products->pluck('id')->toArray()))
                                <a href="{{ route('user.removeFromWishlist',['product_id' => $oneproduct->id, 'user_id' => Auth::user()->id]) }}"
                                onclick="event.preventDefault();document.getElementById('wishlistRemovecard-form{{ $oneproduct->id }}').submit();">
                                <i class="bi bi-heart-fill heart mt-3"></i>
    
                                <form action="{{ route('user.removeFromWishlist',['product_id' => $oneproduct->id, 'user_id' => Auth::user()->id]) }}" 
                                method="POST" id="wishlistRemovecard-form{{ $oneproduct->id }}" class="d-none">@method('delete')@csrf</form></a>
    
                            @else
                                <a href="{{ route('user.addToWishlist',['product_id' => $oneproduct->id , 'user_id' => Auth::user()->id]) }}" 
                                onclick="event.preventDefault();document.getElementById('wishlistAddcard-form{{ $oneproduct->id }}').submit();">
                                <i class="bi bi-heart heart mt-3"></i></a> 
    
                                <form action="{{ route('user.addToWishlist',['product_id' => $oneproduct->id , 'user_id' => Auth::user()->id]) }}" 
                                method="POST" id="wishlistAddcard-form{{ $oneproduct->id }}" class="d-none">@method('post')@csrf</form></a>  
                            @endif
                        @else
                        <a href="{{ route('user.Login') }}"><i class="bi bi-heart heart mt-3"></i></a>
                        @endauth
                            <a href="{{ route('ShowProduct',$oneproduct->id) }}" class="text-decoration-none"><img src="{{ asset('productImages/'.$oneproduct->imagepath) }}" class="card-img-top "  height="110px"></a>
    
                            
                            <div class="card-body">
                                <h6 class="card-title">{{ $oneproduct->name }}</h6>
                                <p class="card-text">Rs.{{ number_format($oneproduct->price) }}.00/Unit</p>
    
                                <div class="cart">
                                    <a href="{{ route('ShowProduct',$oneproduct->id) }}" id="moveTrashbutton" class="text-black text-decoration-none moveTrashbutton"><i class="bi-cart-plus-fill text-black actionIcon" style="cursor: pointer"></i></a>
                                </div>
                            </div>
                        </div>
                    </div> 
                 @endforeach             
            </div>
        </div>
        
        <div class="container">
            <div class="row">
                <div class="col-md-12">
                    <div class="d-flex justify-content-center">
                        {{ $product->links(); }}
                    </div>
                </div>
            </div>
        </div>
    </section>

@endsection

@section('javascript')
<script>
$(document).ready(function (){
    $('#ProCard').on('click','.moveQuantity',function(){ 
        $tr = $(this).closest('tr');

        var data = $tr.children("td").map(function() {
            return $(this).text();
        }).get();
        $('#QuantityForm').attr('action', '/admin/products/' + data[0]);
        $('#qtyhModal').modal('show');
    });
});
</script>
@endsection