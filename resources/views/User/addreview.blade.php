    ...
    <div class="row justify-content-center align-items-center">
      <div class="col-lg-7 shadow reviewBox">
        <div class="p-5 reviewPading">
          <form action="{{ route('user.storeReview') }}" method="POST" autocomplete="off">
              @csrf
            <div class="mb-4 reviewHeadingBox">
              <h2 class="reviewHeding">Write Review</h2>
            </div>
            <div class="itemDetails">
              <img src="{{ asset('productImages/'.$product->imagepath) }}" class="img-fluid mx-auto d-block border border-dark mb-2" width="400">
              <div class="itemDetails text-center">
                <h5 class="reviewItemName">{{ $product->name }}</h5>
                <h6 class="reviewItemPrice">Rs.{{ number_format($product->price) }}</h6>
              </div>
            </div>
